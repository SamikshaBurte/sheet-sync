...
def get_rows():
    ...
    rows = [{"id": f"r{i + 1}", "a": (row + ["", "", ""])[0], "b": (row + ["", "", ""])[1], "c": (row + ["", "", ""])[2]} for i, row in enumerate(body[:4])]
    return {"rows": rows, "mode": "live"}

@app.put("/")
def put_rows(payload: RowsPayload):
    if len(payload.rows) != 4:
        raise HTTPException(400, "Exactly four rows are required")
    values = [["A", "B", "C"]] + [[row.a, row.b, row.c] for row in payload.rows]
    sheets_service().spreadsheets().values().update(
        spreadsheetId=SHEET_ID, range=RANGE, valueInputOption="USER_ENTERED", body={"values": values}
    ).execute()
